...
import {
  getPrioritizedRedirectUris,
  logDomainInfo,
} from "../utils/domainDetector";
...
export class LinkedInProvider {
  ...
  generateAuthUrl(req: Request): string {
    ...
    const redirectUris = getPrioritizedRedirectUris(
      req,
    ...
    const state =
      this.config.state || Math.random().toString(36).substring(2, 15);
  ...
  handleCallback(
    ...
    return async (req: Request, res: Response, next: NextFunction) => {
      ...
      const oauthState = req.session?.linkedinOAuth as
        | LinkedInOAuthState
        | undefined;
      ...
      try {
        ...
        if (
          oauthState &&
          oauthState.fallbackUris.length > 0 &&
          oauthState.attemptCount <= 3
        ) {
          console.log(
            `Retrying with fallback URI, attempt ${oauthState.attemptCount}`,
            {
